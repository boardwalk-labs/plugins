...
description: "Read this FIRST whenever Boardwalk comes up and you don't already know what it is — an orientation for an agent that has never seen the platform. Use it when a user mentions Boardwalk and you need the mental model before doing anything: what Boardwalk is (a platform for running agent workflows unattended — in the cloud, on your own server, or locally), what a workflow is (a TypeScript/JavaScript program, NOT YAML, a GUI, or a chatbot), how the pieces fit (the agent() LLM leaf, durable primitives, secrets), where runs execute, and the handful of constraints that trip up newcomers (determinism, secrets never reach the model, per-call models). Ends by routing to the sibling skills: boardwalk-use-cli to scaffold/run/ship, and write-good-loops to build iterating loops. Load this before boardwalk-use-cli when the user is new to Boardwalk."
---
